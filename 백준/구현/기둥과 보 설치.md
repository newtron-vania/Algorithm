...
- 명령을 사용했을 때 안되는 경우들을 하나하나 다 넣으려고 했는데 배보다 배꼽이 더 커지는 것 같은 기분이다.
```python
import sys
...
def isValid(commands):
    x, y, s, c = commands
    if c == 0:
        if s == 1:
            if (x,y, 0) not in answer and (x-1,y,1) in answer and (x-1,y-1,0) not in answer:
                return False
            elif (x+1,y, 1) in answer and (x+1,y-1,0) not in answer and (x+2, y-1, 0) not in answer:
                return False
            elif (x,y, 0) in answer and (x-1,y,1) not in answer and (x,y-1,0) not in answer:
                return False
            elif (x_1, y, 0) in answer and (x+1,y,1) not in answer and (x+1,y,0) not in answer:
                return False            
        else:
            if(x,y,1) in answer:
                if (x-1,y,1) not in answer and ((x+1, y, 1) not in answer or (x+1, y-1, 0) not in answer):
                    return False
    else:
        if s==1:
...
def solution(n, build_frame):
    answer = set()
    for commands in build_frame:
        #x, y ,structure, commander
        x, y, s, c = commands
        if c == 0:
            answer.remove(cmd)
            #if check
...
- 결과물을 집어넣은 후 전체에서 문제가 없는지 확인하는 방식으로 바꿨다. 코드량이 줄어들었고 조건문 안에서 answer를 호출하는 양이 많이 줄어들었다.

```python
def isValid(answer):
    for x,y,a in answer:
        if a==0:
            #아래에 기둥이 없거나 기둥 좌측 혹은 자기위치에 보가 있다면
            if (x,y-1,0) in answer or (x-1,y,1) in answer or (x,y,1) in answer or y==0:
                continue
            else:
                return False
        if a==1:
            #아래 혹은 우측 아래에 기둥이 있거나 좌우로 보가 존재한다면
