...
#bfs를 이용해서 클러스터를 찾고, 클러스터가 바닥과 연결되어 있지 않다면 가장 짧은 수직거리만큼 클러스터를 떨어뜨리는 것이 중요하다. 다만, 이 식의 경우 클러스터가 C의 모양일 경우 그 사이에 미네랄이 존재할 경우를 생각하지 못한 경우이기에 다시한번 고칠 필요가 있다.
...
def bfs(x, y, graph, visited):
    ...
    #최소 수직 길이 찾기
    min_height = 1e9
    for idx, minerals in enumerate(cluster):
        if len(minerals) == 0:
    	    continue
        min_height_in_idx = sorted(minerals)[0]
        for i in range(min_height_in_idx-1, -1, -1):
            if graph[i][idx] == "x":
                min_height = min(min_height, min_height_in_idx - i-1)
    #최소 수직길이만큼 클러스터 이동
    for idx, val in enumerate(cluster):
        for i in val:
