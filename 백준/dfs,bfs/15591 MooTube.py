...
==================================
'''
#BFS를 이용한 문제. 다음 노드와의 유사도와 현재 유사도 중 최솟값을 k와 비교하여 k보다 크면 다음 노드와 최솟값을 큐에 저장하는 것을 반복한다.
...
def bfs(v, k, n, arr):
    ...
    while queue:
        ...
        #현재 노드와 연결된 다른 노드와 유사도값 참조
        for next_node, dist in arr[node]:
            #이미 등록된 곳이라면 참조할 필요가 없다.
            if visited[next_node] == 1: 
                continue
            #현재 노드의 유사도와 연결 노드의 유사도 비교
            min_dist = min(cur_dist, dist)
            #최소 유사도값이 k보다 크면 다음 노드로 이동
            if min_dist >= k:
                queue.append([next_node, min_dist])
                count += 1
                visited[next_node] = 1
    return count
...
#각 노드 인덱스마다 연결노드와 유사도를 저장한다.
for _ in range(n-1):
    p, q, usado = map(int, input().split())
    arr[p].append([q,usado])
    arr[q].append([p,usado])
for i in range(m):
