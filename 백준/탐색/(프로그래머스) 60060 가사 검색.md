...
## 문제풀이
- 문자열의 이진탐색 문제다.
- ?가 접두사 혹은 접미사로만 주어지므로 ?가 접미사일 경우를 탐색할 정렬 words와 접두사일 경우를 탐색할 reverse 정렬한 words를 생성한다.
- 정렬 후 문자열을 비교하여 탐색한 word의 가장 왼쪽 index와 오른쪽 index를 탐색한 word의 개수를 answer에 삽입한다.
- ?로만 이루어진 query의 경우 query의 길이에 해당되는 단어들의 개수를 바로 삽입한다.  
